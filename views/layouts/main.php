...
} elseif (strpos($currentRoute, 'terminal') !== false) {
    $initialTab = 'terminal';
    ...
    <div class="flex flex-col h-screen w-screen bg-slate-950 text-slate-100 overflow-hidden" 
        ...
        <div class="flex flex-grow w-screen overflow-hidden relative transition-all duration-300"
            ...
            <main class="flex-1 bg-slate-950 overflow-hidden relative">
                ...
                <!-- Gatilho de Carregamento Assíncrono Invisível HTMX para a Aba de Tickets -->
                <button id="btn-nav-tickets"
                        hx-get="<?= Url::to(['/tickets/default/index']) ?>"
                        hx-target="#container-tickets"
                        hx-trigger="click once"
                        class="hidden">
                </button>

                <!-- Aba Tickets: Suporte & Governança (Injeção SPA) -->
                <div class="h-full w-full absolute inset-0 overflow-y-auto scrollbar-thin p-6 md:p-10 bg-slate-950" 
                     x-show="activeTab === 'tickets'"
                     id="container-tickets"
                     <?= $initialTab === 'tickets' ? 'hx-isomorphic="true"' : '' ?>>
                     <?php if ($initialTab === 'tickets'): ?>
                         <?= $content ?>
                     <?php else: ?>
                         <div class="flex items-center justify-center h-full text-slate-500 text-sm">
                              <div class="flex flex-col items-center gap-2">
...
<script>
    ...
    // === CromIA Gateway: Componente Alpine.js Global ===
    // Definido no layout principal para garantir disponibilidade antes de qualquer swap HTMX.
    // Alpine.js mutation observer detecta x-data ANTES de HTMX avaliar <script> tags inline,
    // portanto a função DEVE existir globalmente no momento do carregamento da página.
